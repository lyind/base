...
{
    private final static char[] vocals = {'a', 'e', 'i', 'o', 'u', 'ä', 'ö', 'õ', 'ü', 'y'};

    private final static char[] consonants = {'b', 'c', 'd', 'f', 'g', 'h', 'j', 'k', 'l', 'm', 'n', 'p', 'q', 'r', 's', 't', 'v', 'w', 'x', 'y'};

    private final ArrayList<String> pre = new ArrayList<>();

    private final ArrayList<String> mid = new ArrayList<>();

    private final ArrayList<String> sur = new ArrayList<>();
     ...
     *
     * @param syllables the syllables (same syntax as in the definition file).
     */
    public NameGenerator(String[] syllables) throws IOException
    {
        try (final BufferedReader reader = new BufferedReader(new StringReader(String.join("\n", syllables)));)
        {
            refresh(reader);
        }
    }
    ...
    /**
     * Load new syllables from the specified file.
     *
     * @param fileName The paht of the file which contains the syllables to load.
     */
    ...
    public String compose(int syls)
    {
        if (syls > 2 && mid.size() == 0)
            throw new RuntimeException("You are trying to create a name with more than 3 parts, which requires middle parts, " +
                    "which you did not specify. You should add some. Every word, which doesn't have + or - for a prefix is counted as a middle part.");
        if (pre.size() == 0)
        ...
        for (int i = 0; i < b.length - 2; i++)
        {
            ...
            if (vocalLast(pureSyl(mid.get(b[i]))) && !allowVocals(mid) && syls > 3)
